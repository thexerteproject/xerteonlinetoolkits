...
# ---------------------------------------------------------------------------
# 4b. ClamAV: refresh signatures + generate extra_config.php to enable scanning
# ---------------------------------------------------------------------------
# config.php forces enable_clamav_check=false unless extra_config.php exists, so
# we generate that file (the supported XOT extension point) to turn scanning on.
if [ "${XERTE_ENABLE_CLAMAV}" = "true" ]; then
    # Best-effort signature refresh. Don't let a missing network block startup.
    if [ "${XERTE_FRESHCLAM_ON_START}" = "true" ] && command -v freshclam >/dev/null 2>&1; then
        log "Refreshing ClamAV signatures (best effort) ..."
        freshclam --no-dns --datadir=/var/lib/clamav --no-warn >/dev/null 2>&1 \
            || log "  freshclam refresh failed (continuing with existing signatures)"
    fi

    cat > "${XERTE_DOCROOT}/extra_config.php" <<PHP
<?php
/**
 * Auto-generated by the Xerte Docker entrypoint.
 * Enables ClamAV scanning of uploaded files.
 */
global \$xerte_toolkits_site;
\$xerte_toolkits_site->enable_clamav_check = 'true';
\$xerte_toolkits_site->clamav_cmd         = '$(esc "${XERTE_CLAMAV_CMD}")';
\$xerte_toolkits_site->clamav_opts        = '$(esc "${XERTE_CLAMAV_OPTS}")';
PHP
    chown www-data:www-data "${XERTE_DOCROOT}/extra_config.php" 2>/dev/null || true
    log "ClamAV scanning enabled (extra_config.php written; cmd=${XERTE_CLAMAV_CMD})"
else
    rm -f "${XERTE_DOCROOT}/extra_config.php" 2>/dev/null || true
    log "ClamAV scanning disabled (XERTE_ENABLE_CLAMAV != true)"
fi
