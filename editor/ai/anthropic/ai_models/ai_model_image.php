...
_load_language_file("/editor/ai_models/anthropic_model_image_ai.inc");
...
if ($context === 'standard') {
    $q = LEARNING_PROMPT_IMAGE;
    $object = LEARNING_RESULT_IMAGE;
    $defaultPrompt = DEFAULT_PROMPT_IMAGE;
}
elseif ($context === 'bootstrap') {
    $q = LEARNING_PROMPT_IMAGE_BOOTSTRAP;
    $object = LEARNING_RESULT_IMAGE_BOOTSTRAP;
    $defaultPrompt = DEFAULT_PROMPT_IMAGE_BOOTSTRAP;
}
...
$anthropic_preset_models->type_list["image"] = ["payload" => $payload, "url" => $chat_url];

$anthropic_preset_models->prompt_list["image"] = explode(",", $defaultPrompt);

$anthropic_preset_models->multi_run[] = "image";