...
_load_language_file("/editor/ai_models/anthropic_model_summary_ai.inc");
...
if ($context === 'standard') {
    $q = LEARNING_PROMPT_SUMMARY;
    $object = LEARNING_RESULT_SUMMARY;
    $defaultPrompt = DEFAULT_PROMPT_SUMMARY;
}
elseif ($context === 'bootstrap') {
    $q = LEARNING_PROMPT_SUMMARY_BOOTSTRAP;
    $object = LEARNING_RESULT_SUMMARY_BOOTSTRAP;
    $defaultPrompt = DEFAULT_PROMPT_SUMMARY_BOOTSTRAP;
}
...
$anthropic_preset_models->type_list["summary"] = ["payload" => $payload, "url" => $chat_url];

$anthropic_preset_models->prompt_list["summary"] = explode(",", $defaultPrompt);

$anthropic_preset_models->multi_run[] = "summary";