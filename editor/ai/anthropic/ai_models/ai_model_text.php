...
_load_language_file("/editor/ai_models/anthropic_model_text_ai.inc");
...
if ($context === 'standard') {
    $q = LEARNING_PROMPT_TEXT;
    $object = LEARNING_RESULT_TEXT;
    $defaultPrompt = DEFAULT_PROMPT_TEXT;
}
elseif ($context === 'bootstrap') {
    $q = LEARNING_PROMPT_TEXT_BOOTSTRAP;
    $object = LEARNING_RESULT_TEXT_BOOTSTRAP;
    $defaultPrompt = DEFAULT_PROMPT_TEXT_BOOTSTRAP;
}
...
$anthropic_preset_models->type_list["text"] = ["payload" => $payload, "url" => $chat_url];

$anthropic_preset_models->prompt_list["text"] = explode(",", $defaultPrompt);

$anthropic_preset_models->multi_run[] = "text";