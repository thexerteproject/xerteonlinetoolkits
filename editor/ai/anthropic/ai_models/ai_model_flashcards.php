...
_load_language_file("/editor/ai_models/anthropic_model_flashcards_ai.inc");
...
if ($context === 'standard') {
    ...
    $defaultPrompt = DEFAULT_PROMPT_FLASHCARDS;
}
elseif ($context === 'bootstrap') {
    $q = LEARNING_PROMPT_FLASHCARDS_BOOTSTRAP;
    $object = LEARNING_RESULT_FLASHCARDS_BOOTSTRAP;
    $defaultPrompt = DEFAULT_PROMPT_FLASHCARDS_BOOTSTRAP;
}
...
$anthropic_preset_models->type_list["flashCards"] = ["payload" => $payload, "url" => $chat_url];

$anthropic_preset_models->prompt_list["flashCards"] = explode(",", $defaultPrompt);

$anthropic_preset_models->multi_run[] = "flashCards";