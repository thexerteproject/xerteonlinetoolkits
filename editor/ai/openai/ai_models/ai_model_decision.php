...
_load_language_file("/editor/ai_models/openai_model_decision_ai.inc");
...
$modelTemplate = $_POST['modelTemplate'] ?? "standard";
...
if ($context === 'standard') {
    if ($modelTemplate == 'suggestion'){
        $q = LEARNING_PROMPT_DECISION_SUGGEST;
        $object = LEARNING_RESULT_DECISION_SUGGEST;
        $defaultPrompt = DEFAULT_PROMPT_DECISION_SUGGEST;
    }elseif ($modelTemplate == 'construct'){
        $q = LEARNING_PROMPT_DECISION_CONSTRUCT;
        $object = LEARNING_RESULT_DECISION_CONSTRUCT;
        $defaultPrompt = DEFAULT_PROMPT_DECISION_CONSTRUCT;
    }else{
        $q = LEARNING_PROMPT_DECISION;
        $object = LEARNING_RESULT_DECISION;
        $defaultPrompt = DEFAULT_PROMPT_DECISION;
    }
}
elseif ($context === 'bootstrap') {
    $q = LEARNING_PROMPT_DECISION_SUGGEST_BOOTSTRAP;
    $object = LEARNING_RESULT_DECISION_SUGGEST_BOOTSTRAP;
    $defaultPrompt = DEFAULT_PROMPT_DECISION_SUGGEST_BOOTSTRAP;
}
if ($assistantOn){
...
$openAI_preset_models->type_list["decision"] = ["payload" => $payload, "url" => $chat_url];

$openAI_preset_models->prompt_list["decision"] = explode(",", $defaultPrompt);

$openAI_preset_models->multi_run[] = "decision";