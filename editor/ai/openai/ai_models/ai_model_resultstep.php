...
_load_language_file("/editor/ai_models/openai_model_resultstep_ai.inc");
...
if ($context === 'standard') {
        $q = LEARNING_PROMPT_RESULTSTEP;
        $object = LEARNING_RESULT_RESULTSTEP;
        $defaultPrompt = DEFAULT_PROMPT_RESULTSTEP;

}
elseif ($context === 'bootstrap') {
    $q = LEARNING_PROMPT_RESULTSTEP_BOOTSTRAP;
    $object = LEARNING_RESULT_RESULTSTEP_BOOTSTRAP;
    $defaultPrompt = DEFAULT_PROMPT_RESULTSTEP_BOOTSTRAP;
}
if ($assistantOn){
...
$openAI_preset_models->type_list["resultStep"] = ["payload" => $payload, "url" => $chat_url];

$openAI_preset_models->prompt_list["resultStep"] = explode(",", $defaultPrompt);

$openAI_preset_models->multi_run[] = "resultStep";