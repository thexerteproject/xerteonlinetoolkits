...
_load_language_file("/editor/ai_models/openai_model_infostep_ai.inc");
...
if ($context === 'standard') {
        $q = LEARNING_PROMPT_INFOSTEP;
        $object = LEARNING_RESULT_INFOSTEP;
        $defaultPrompt = DEFAULT_PROMPT_INFOSTEP;

}
elseif ($context === 'bootstrap') {
    $q = LEARNING_PROMPT_INFOSTEP_BOOTSTRAP;
    $object = LEARNING_RESULT_INFOSTEP_BOOTSTRAP;
    $defaultPrompt = DEFAULT_PROMPT_INFOSTEP_BOOTSTRAP;
}
if ($assistantOn){
...
$openAI_preset_models->type_list["infoStep"] = ["payload" => $payload, "url" => $chat_url];

$openAI_preset_models->prompt_list["infoStep"] = explode(",", $defaultPrompt);

$openAI_preset_models->multi_run[] = "infoStep";