...
CKEDITOR.dialog.add( 'image3', function( editor ) {
	...
	var regexGetSizeOrEmpty = /(^\s*(\d+)(px)?\s*$)|^$/i,
		regexGetSizeOrEmptyPerc = /(^\s*(\d+)%\s*$)/i,
	...
	function validateDimension() {
		var match = this.getValue().match( regexGetSizeOrEmpty ),
			matchperc = this.getValue().match( regexGetSizeOrEmptyPerc ),
			isValid = !!( (match && parseInt( match[ 1 ], 10 ) !== 0)
						  || (matchperc && parseInt( matchperc[ 1 ], 10 ) !== 0));
	...
	function onChangeDimension() {
		...
		// If the value of the field matches number with %, set other value to same percentage
		if (value.match( regexGetSizeOrEmptyPerc ))
		{
            // apply it to the other field.
		...
		var	width = domWidth || preLoadedWidth,
