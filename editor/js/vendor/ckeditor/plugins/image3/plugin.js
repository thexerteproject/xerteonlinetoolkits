...
( function() {
		...
		return {
			...
			init: function() {
				var helpers = CKEDITOR.plugins.image3,
                    ...
                    hasPerc = !!(dimensions['width'].match(regexPercent) || dimensions['height'].match(regexPercent));
				...
				// If the size is expressed in percentages, initilise data in the correct way if needed
				if (hasPerc && data.hasCaption)
				{
					data['width'] = alignElement.getStyle( 'width' );
					data['height'] = alignElement.getStyle( 'height' );
					alignElement.removeStyle( 'width' );
					alignElement.removeStyle( 'height' );
				}
				...
				if ( editor.filter.checkFeature( this.features.dimension ) && editor.config.image3_disableResizer !== true )
					setupResizer( this );
	...
	function downcastWidgetElement( editor ) {
		...
		return function( el ) {
			...
			if (hasPerc)
			{
				// Figure out where to put the width and height percentages
				if (hasCaption)
				{
	...
	function setDimensions( widget ) {
		...
		if (widget.wrapper != null) {
            var styles = CKEDITOR.tools.parseCssText(widget.wrapper.getAttribute('style') || '', true);
            for (var d in dimensions) {
                if (dimensions[d]) {
                    if (styles[d])
                        delete styles[d];
		...
		// Write back styles to warpper
		if (widget.wrapper != null)
		{
			widget.wrapper.setAttribute('style', CKEDITOR.tools.writeCssText(styles));
        }
	}
