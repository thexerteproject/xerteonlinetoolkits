...
if(is_user_permitted("projectadmin")) {
    ...
    $query_templates = "select td.*, tr.*, ld.*, od.login_id as owner_id, od.firstname "
        ...
        . "and od.login_id = td.creator_id and tr.template_id = td.template_id and td.template_id=?";
    $params = array($login_id, $template_id);

    _debug("Query for templates of user " . $row['username'] . ": " . $query_templates);

    $row_templates = db_query_one($query_templates, $params);
    ...
    echo "</select>";
    echo "<input type=\"hidden\" value=\"" . $row['login_id'] . "_" . $row_templates['template_id'] . "\" name=\"template_id\" /><button type=\"submit\" class=\"xerte_button\"><i class=\"fa fa-share\"></i> " . USERS_MANAGEMENT_TEMPLATE_GIVE_BUTTON . "</button></form>";
