...
l.firstChild.nodeValue},0);return a("#"+k,f)}c.appendChild(g);p='<video id="'+k+'" preload=auto autobuffer>';d=0;for(n=b.length;d<n;d++)p+='<source src="'+b[d]+'">';p+="</video>";g.innerHTML=p;f&&f.events&&f.events.error&&c.addEventListener("error",f.events.error,false);return a("#"+k,f)}else a.error("Specified target `"+d+"` was not found.")}})(Popcorn);(function(a){document.addEventListener("DOMContentLoaded",function(){var t=document.querySelectorAll("[data-timeline-sources]");a.forEach(t,function(d,b){var f=t[b],c,i,g;if(!f.id)f.id=a.guid("__popcorn");if(f.nodeType&&f.nodeType===1){g=a("#"+f.id);c=(f.getAttribute("data-timeline-sources")||"").split(",");c[0]&&a.forEach(c,function(j){i=j.split("!");if(i.length===1){i=j.match(/(.*)[\/\\]([^\/\\]+\.\w+)$/)[2].split(".");i[0]="parse"+i[1].toUpperCase();i[1]=j}c[0]&&g[i[0]]&&g[i[0]](i[1])});g.autoplay()&&
g.play()}})},false)})(Popcorn);
